...
async def test_event_store():
    ...
    result_stream_id = await event_store.replay_events_after(first_event_id, replay_callback)
    ...
    if result_stream_id:
        ...
        if len(replayed_events) == expected_count:
            ...
            logger.error(f"❌ Event replay test failed: expected {expected_count}, got {len(replayed_events)}")
    else:
    ...
    if stream_info_after_cleanup is None or stream_info_after_cleanup.get("event_count", 0) == 0:
