...
EventCallback = Callable[[EventMessage], Any]  # Made non-async for now, can be updated if needed
...
class EventStore(ABC):
    ...
    async def store_event(
        self, stream_id: StreamId, message: Any
    ) -> EventId:
        """
...
class SQLiteEventStore(EventStore):
    ...
    async def store_event(
        ...
        try:
            ...
            cursor.execute(
                "INSERT OR IGNORE INTO streams (stream_id) VALUES (?)",
                (stream_id,)
            )
            ...
            try:
                ...
                message_data = json.dumps({"error": "serialization_failed", "message": str(message)})
            ...
            conn.commit()
            logger.debug(f"Stored event {event_id} for stream {stream_id} (seq: {sequence_number})")
            return event_id
    ...
    def cleanup_old_events(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"Cleaned up {deleted_events} old events and {deleted_streams} empty streams")
