...
try:
    from mcp_modelservice_sdk.cli import app as cli_app # 'app' is the Typer instance in cli.py
    ...
    print(f"CRITICAL: Could not import from mcp_modelservice_sdk.cli. Ensure package is installed or PYTHONPATH is correct. Error: {e}")
...
""")
    ...
    def test_run_successful_minimum_args(self, mock_create_app, mock_uvicorn_run):
        ...
        result = runner.invoke(cli_app, ["run", "--source-path", str(self.dummy_source_file)])
        
        self.assertEqual(result.exit_code, 0, msg=f"CLI failed with: {result.stdout}{result.stderr}")
    ...
    def test_run_with_custom_params(self, mock_create_app, mock_uvicorn_run):
        ...
        result = runner.invoke(cli_app, [
            "run",
            "--source-path", str(self.dummy_source_file),
            "--host", "192.168.1.100",
            "--port", "9090",
            "--log-level", "debug",
            "--functions", "a_func,b_func", # Comma-separated
            "--mcp-name", "MyTestService",
            "--server-root", "/api",
            "--mcp-base", "/service",
            "--no-mw-service", # Explicitly disable mw_service to use provided host/port
            "--reload",
            "--workers", "2",
            "--cors-allow-origins", "http://localhost:3000,http://example.com"
        ])
        self.assertEqual(result.exit_code, 0, msg=f"CLI failed with: {result.stdout}{result.stderr}")
        ...
        self.assertEqual(kwargs['cors_allow_origins'], ["http://localhost:3000", "http://example.com"])
    ...
    def test_run_transformation_error(self, mock_create_app):
        ...
        self.assertIn("Failed to create MCP application: Test transformation failed", result.stdout)
        self.assertIn("Please check the source path and function definitions.", result.stdout)
    ...
    def test_run_invalid_source_path_cli_level(self):
        ...
        with patch('mcp_modelservice_sdk.cli.create_mcp_application') as mock_create_app:
            mock_create_app.side_effect = FileNotFoundError("Path does not exist")
            result = runner.invoke(cli_app, ["run", "--source-path", "/tmp/nonexistent123.py"])
            self.assertNotEqual(result.exit_code, 0)
            self.assertIn("Error: Path does not exist", result.stdout)

    @patch('mcp_modelservice_sdk.cli.uvicorn.run')
    @patch('mcp_modelservice_sdk.cli.create_mcp_application')
    def test_run_functions_single_item_in_list_no_comma(self, mock_create_app, mock_uvicorn_run):
        mock_create_app.return_value = MagicMock()
        result = runner.invoke(cli_app, [
            "run",
            "--source-path", str(self.dummy_source_file),
