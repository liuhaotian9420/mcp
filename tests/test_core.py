...
try:
    ...
    print("CRITICAL: Could not import from mcp_modelservice_sdk.src.core. Ensure package is installed or PYTHONPATH is correct.")
...
class TestCoreDiscoverPyFiles(unittest.TestCase):
    ...
    def test_discover_no_py_file_extension(self):
        ...
        self.assertIn(f"Source file is not a Python file, skipping: {file_path}", log.output[0])
...
class TestCoreDiscoverFunctions(unittest.TestCase):
    ...
    def test_discover_specific_functions_single_file(self):
        funcs = discover_functions([self.module1_path], target_function_names=["func_a"])
    ...
    def test_discover_non_existent_function(self):
        with self.assertLogs(level='WARNING') as log:
            funcs = discover_functions([self.module1_path], target_function_names=["non_existent_func"])
            self.assertEqual(len(funcs), 0)
        self.assertIn("Could not find the following specified functions: ['non_existent_func']", log.output[0])
    ...
    def test_file_cannot_be_loaded(self):
        ...
        with self.assertLogs(level='ERROR') as log:
            ...
            self.assertEqual(len(funcs), 0) # No functions should be discovered from a broken module
        self.assertTrue(any(f"Failed to load module 'invalid' from '{invalid_module_path}'" in record for record in log.output))
...
class TestCoreValidateAndWrapTool(unittest.TestCase):
    ...
    def test_missing_docstring(self):
        def sample_func(a):
            pass
        with self.assertLogs(logger='mcp_modelservice_sdk.src.core', level='WARNING') as log:
            _validate_and_wrap_tool(self.mcp_instance, sample_func, "sample_func", self.test_file_path)
        self.assertIn(f"Function 'sample_func' in '{self.test_file_path}' is missing a docstring.", log.output[0])
    ...
    def test_missing_param_type_hint(self):
        ...
        with self.assertLogs(logger='mcp_modelservice_sdk.src.core', level='WARNING') as log:
            _validate_and_wrap_tool(self.mcp_instance, sample_func, "sample_func", self.test_file_path)
        self.assertIn(f"Parameter 'a' in function 'sample_func' in '{self.test_file_path}' is missing a type hint.", log.output[0])
        # Check that b is not warned for
        self.assertFalse(any(f"Parameter 'b'" in line for line in log.output))
    ...
    def test_missing_return_type_hint(self):
        ...
        with self.assertLogs(logger='mcp_modelservice_sdk.src.core', level='WARNING') as log:
            _validate_and_wrap_tool(self.mcp_instance, sample_func, "sample_func", self.test_file_path)
        self.assertIn(f"Return type for function 'sample_func' in '{self.test_file_path}' is missing a type hint.", log.output[0])
    ...
    def test_all_present(self):
        ...
        with patch.object(logging.getLogger('mcp_modelservice_sdk.src.core'), 'warning') as mock_warning:
            _validate_and_wrap_tool(self.mcp_instance, sample_func, "sample_func", self.test_file_path)
    ...
    def test_wrapping_failure(self, mock_mcp_tool_decorator):
        ...
        with self.assertLogs(logger='mcp_modelservice_sdk.src.core', level='ERROR') as log:
            _validate_and_wrap_tool(self.mcp_instance, sample_func, "sample_func", self.test_file_path)
        
        self.assertTrue(any(f"Failed to wrap function 'sample_func' from '{self.test_file_path}' as an MCP tool: Wrapping Failed" in record for record in log.output))
...
class TestCoreCreateMcpApplication(unittest.TestCase):
    ...
    def test_create_app_successfully(self):
        ...
        with patch('mcp_modelservice_sdk.src.core.FastMCP') as mock_fast_mcp_class:
            ...
            mock_mcp_instance.http_app.assert_called_with(path="/mcp") # Default base path
    ...
    def test_specific_functions_not_found_error(self):
        with self.assertRaises(TransformationError) as cm:
            create_mcp_application(
                str(self.dummy_module_path), 
                target_function_names=["non_existent_tool"]
            )
        self.assertIn("Specified functions: ['non_existent_tool'] not found", str(cm.exception))
    ...
    def test_no_tools_registered_error(self, mock_validate_wrap):
        ...
        with patch('mcp_modelservice_sdk.src.core.FastMCP') as mock_fast_mcp_class:
            ...
            with patch('mcp_modelservice_sdk.src.core.discover_functions') as mock_discover:
                def dummy_f(): pass
                mock_discover.return_value = [(dummy_f, "dummy_f", self.dummy_module_path)]

                with self.assertRaises(TransformationError) as cm:
                    create_mcp_application(str(self.dummy_module_path))
                self.assertIn("No tools were successfully created and registered", str(cm.exception))
...
""")
    ...
    def assert_file_contains(self, file_path: pathlib.Path, expected_content: str):
        ...
        with open(file_path, "r") as f:
            content = f.read()
        self.assertIn(expected_content, content, f"Expected content not found in {file_path}")

    def assert_file_does_not_contain(self, file_path: pathlib.Path, unexpected_content: str):
        self.assertTrue(file_path.exists(), f"{file_path} does not exist")
        with open(file_path, "r") as f:
            content = f.read()
        self.assertNotIn(unexpected_content, content, f"Unexpected content found in {file_path}")
    ...
    def test_package_mcp_application_defaults(self):
        ...
        self.assertTrue((self.output_dir / "app" / f"{self.dummy_module_name}.py").exists())
        ...
        self.assert_file_contains(main_py_path, f"from app.{self.dummy_module_name} import") # Correct import
        self.assert_file_contains(main_py_path, "create_mcp_application(source_path_str=module_path_str,")
        self.assert_file_contains(main_py_path, 'mcp_app = create_mcp_application(source_path_str=str(module_path), mcp_server_name="MyMCPApp",')
        self.assert_file_contains(main_py_path, 'uvicorn.run(mcp_app, host="0.0.0.0", port=8080)')


        # Check Dockerfile
        dockerfile_path = self.output_dir / "Dockerfile"
        self.assert_file_contains(dockerfile_path, "FROM python:3.10-slim") # Default Python version
        self.assert_file_contains(dockerfile_path, "COPY ./app /app/app")
        self.assert_file_contains(dockerfile_path, "COPY requirements.txt /app/requirements.txt")
        self.assert_file_contains(dockerfile_path, "RUN pip install --no-cache-dir -r /app/requirements.txt")
        self.assert_file_contains(dockerfile_path, 'CMD ["uvicorn", "main:mcp_app", "--host", "0.0.0.0", "--port", "8080"]')
        ...
        self.assert_file_contains(readme_path, f"Based on module: {self.dummy_module_name}.py")
        ...
        self.assert_file_does_not_contain(req_path, "requests") # example of an extra dep
    ...
    def test_package_mcp_application_custom_params(self):
        ...
        main_py_path = self.output_dir / "main.py"
        self.assert_file_contains(main_py_path, f'mcp_app = create_mcp_application(source_path_str=str(module_path), mcp_server_name="{custom_app_name}",')


        # Check Dockerfile for custom Python version
        dockerfile_path = self.output_dir / "Dockerfile"
        self.assert_file_contains(dockerfile_path, f"FROM python:{custom_python_version}")
...
""")
        ...
        self.assertFalse((app_folder / "subdir" / "notes.txt").exists()) # If we had one
        ...
        self.assert_file_contains(main_py_path, "create_mcp_application(source_path_str=module_path_str,") # module_path_str is "app"
        ...
        self.assert_file_contains(main_py_path, f'mcp_app = create_mcp_application(source_path_str=str(Path("app")), mcp_server_name="MyMCPApp"')
    ...
    def test_package_output_dir_not_empty_no_force(self):
        ...
        with self.assertRaisesRegex(ValueError, "Output directory .* is not empty. Use --force to overwrite."):
            package_mcp_application(str(self.dummy_module_file), str(self.output_dir), force=False)

    def test_package_output_dir_not_empty_with_force(self):
        self.output_dir.mkdir()
        (self.output_dir / "old_main.py").touch() # Pre-existing file
        
        package_mcp_application(str(self.dummy_module_file), str(self.output_dir), force=True)
        
        self.assertTrue((self.output_dir / "main.py").exists()) # New main.py created
        self.assertFalse((self.output_dir / "old_main.py").exists()) # Old file should be gone
        self.assertTrue((self.output_dir / "app" / f"{self.dummy_module_name}.py").exists())
    ...
    def test_package_source_is_not_py_file_or_dir(self):
        ...
        not_py_file.touch()
        with self.assertRaisesRegex(ValueError, "Source path must be a Python file \\(\\.py\\) or a directory containing Python files."):
